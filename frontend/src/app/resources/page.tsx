...
export default function ResourcesPage() {
  ...
  async function handleExtract(resourceID: string) {
    ...
    try {
      ...
      // Re-fetch from the backend so the "Extracted" state persists across
      // reloads instead of only living in local component state.
      await loadResources();
    } catch (err) {
      console.error(err);
    } finally {
            ...
            <Card key={r.ID} className="flex items-center justify-between gap-4">
              ...
              >
                {extractingID === r.ID ? (
                  ...
                  <>
                    <Check size={14} /> Re-extract
                  </>
                ) : (
                  <>
                    <Sparkles size={14} /> Extract concepts
                  </>
                )}
              </Button>
            </Card>
